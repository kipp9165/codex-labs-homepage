      ...
      <div className="grid gap-4 md:grid-cols-3">
        {altitudePricing.map((item) => (
          <article key={item.suite} className="rounded border border-gray-200 bg-white p-4 shadow-sm" style={{ borderColor: '#00C8FF' }}>
            ...
            <a href={item.checkoutUrl} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex rounded px-3 py-2 text-xs font-semibold text-white" style={{ backgroundColor: '#FF6B00' }}>
              Buy via Stripe
            </a>
          </article>
        ))}
